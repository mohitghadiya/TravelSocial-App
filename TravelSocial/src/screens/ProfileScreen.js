...
//           name: data.user.name,
//           avatarUrl:
//             data.user.avatar && data.user.avatar.length > 0
//               ? data.user.avatar
//               : "https://via.placeholder.com/300",
//           bio: data.user.bio || "",
//           followers: data.user.followersCount,
//           following: data.user.followingCount,
//           distance: "—", // will come from trips later
//           streak: "—",   // will come from trips later
//           badges: ["CORE"], // static for now
//         });
//       }
//     } catch (error) {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const endpoint = profileUserId
        ? `${API_BASE_URL}/api/profile/users/${profileUserId}`
      ...
      if (data.success) {
        setIsMyProfile(
          data.isMyProfile !== undefined ? data.isMyProfile : true
        );
